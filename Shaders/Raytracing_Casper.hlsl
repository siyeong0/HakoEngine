...
void MyIntersectionShader_Casper()
{
    ...
    uint axisEnter = 0;
    if (abs(enterPoint.x) > 0.99)
        axisEnter = 0;
    if (abs(enterPoint.y) > 0.99)
        axisEnter = 1;
    if (abs(enterPoint.z) > 0.99)
        axisEnter = 2;
...
void MyClosestHitShader_RadianceRay_Casper(inout RadiancePayload rayPayload, in MyCasperIntersectionAttributes attr)
{
    ...
    float4 texDiffuse = l_DiffuseAtlasTexture.SampleLevel(g_SamplerClamp, -ObjectRayDirection(), 0);
