...
{
	virtual void ENGINECALL CreateFromStaticMesh(const UStaticMesh & mesh, bool bUseRayTracingIfSupported = true) = 0;

	virtual bool ENGINECALL BeginCreateMesh(const Vertex * vertices, uint numVertices, uint numTriGroups) = 0;
	virtual bool ENGINECALL InsertTriGroup(const uint16_t* indices, uint numTriangles, const UMaterial& material) = 0;
