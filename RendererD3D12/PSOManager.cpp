...
PSOHandle* PSOManager::QueryPSO(PSOHandle* handle)
{
	std::scoped_lock lock(m_Mutex);
	PSOHandle* outHandle = nullptr;

	if (auto it = m_PSOMap.find(handle->Hash); it != m_PSOMap.end())
	{
		++(it->second.RefCount);  // Reference Count
		outHandle = &(it->second);
		goto lb_return;
	}
	ASSERT(false, "QueryPSO called but key does not exist. Use CreatePSO instead.");
lb_return:
	return outHandle;
}
