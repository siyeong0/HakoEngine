...
void BasicMeshObject::Draw(int threadIndex, ID3D12GraphicsCommandList* pCommandList, const XMMATRIX* wordMatrix)
{
...
//	return TRUE;
//}
//bool __stdcall BasicMeshObject::BeginCreateMesh(const BasicVertex* pVertexList, uint32_t dwVertexNum, uint32_t dwTriGroupCount)
//{
//	ID3D12Device5* pD3DDeivce = m_pRenderer->GetD3DDevice();
//	D3D12ResourceManager* pResourceManager = m_pRenderer->GetResourceManager();
//	BOOL bUseGpuUploadHepas = m_pRenderer->IsGpuUploadHeapsEnabledInl();
...
//	}
//
//	m_MaxNumTriGroups = dwTriGroupCount;
//	m_pTriGroupList = new IndexedTriGroup[m_MaxNumTriGroups];
//	memset(m_pTriGroupList, 0, sizeof(IndexedTriGroup) * m_MaxNumTriGroups);
//
//	return TRUE;
//}
//bool __stdcall BasicMeshObject::InsertTriGroup(const WORD* pIndexList, uint32_t dwTriCount, const WCHAR* wchTexFileName)
//{
//	ID3D12Device5* pD3DDeivce = m_pRenderer->GetD3DDevice();
//	UINT srvDescriptorSize = m_pRenderer->GetSrvDescriptorSize();
//	D3D12ResourceManager* pResourceManager = m_pRenderer->GetResourceManager();
...
//	D3D12_INDEX_BUFFER_VIEW IndexBufferView = {};
//
//	if (m_NumTriGroups >= m_MaxNumTriGroups)
//	{
//		__debugbreak();
//		return FALSE;
//	}
...
//	pTriGroup->IndexBuffer = pIndexBuffer;
//	pTriGroup->IndexBufferView = IndexBufferView;
//	pTriGroup->NumTriangles = dwTriCount;
//	pTriGroup->pTexHandle = (TextureHandle*)m_pRenderer->CreateTextureFromFile(wchTexFileName);
//	m_NumTriGroups++;
//	return TRUE;
//}
//void __stdcall BasicMeshObject::EndCreateMesh()
//{
//
//}
//void BasicMeshObject::Draw(int dwThreadIndex, ID3D12GraphicsCommandList* pCommandList, const XMMATRIX* pMatWorld)
//{
//	// 각각의 draw()작업의 무결성을 보장하려면 draw() 작업마다 다른 영역의 descriptor table(shader visible)과 다른 영역의 CBV를 사용해야 한다.
//	// 따라서 draw()할 때마다 CBV는 ConstantBuffer Pool로부터 할당받고, 렌더리용 descriptor table(shader visible)은 descriptor pool로부터 할당 받는다.
//
//	ID3D12Device5* pD3DDeivce = m_pRenderer->GetD3DDevice();
//	UINT srvDescriptorSize = m_pRenderer->GetSrvDescriptorSize();
//	DescriptorPool* pDescriptorPool = m_pRenderer->GetDescriptorPool(dwThreadIndex);
//	ID3D12DescriptorHeap* pDescriptorHeap = pDescriptorPool->GetDescriptorHeap();
//	SimpleConstantBufferPool* pConstantBufferPool = m_pRenderer->GetConstantBufferPool(CONSTANT_BUFFER_TYPE_DEFAULT, dwThreadIndex);
//
//
//	CD3DX12_GPU_DESCRIPTOR_HANDLE gpuDescriptorTable = {};
//	CD3DX12_CPU_DESCRIPTOR_HANDLE cpuDescriptorTable = {};
//	DWORD dwRequiredDescriptorCount = DESCRIPTOR_COUNT_PER_OBJ + (m_NumTriGroups * DESCRIPTOR_COUNT_PER_TRI_GROUP);
//
...
//
//	// world matrix
//	pConstantBufferDefault->WorldMatrix = XMMatrixTranspose(*pMatWorld);
//
//	// Descriptor Table 구성
//	// 이번에 사용할 constant buffer의 descriptor를 렌더링용(shader visible) descriptor table에 카피
//
...
//
//	// per tri-group
//	for (DWORD i = 0; i < m_NumTriGroups; i++)
//	{
//		IndexedTriGroup* pTriGroup = m_pTriGroupList + i;
//		TextureHandle* pTexHandle = pTriGroup->pTexHandle;
//		if (pTexHandle)
